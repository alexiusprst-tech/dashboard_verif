...
import { Plus, MinusCircle, Calendar, UserCheck, BookOpen, Search, X, RefreshCw, ShieldCheck, CheckCircle2 } from 'lucide-react';
...
export function PenugasanPicPage() {
    ...
    useEffect(() => {
        ...
        if (isSuperAdmin) {
            api.get('/courses', { params: { per_page: 100 } }).then((res) => {
        ...
        try {
            ...
            toast.success(
                data.dosen_ids.length > 1
                    ? `${data.dosen_ids.length} Dosen berhasil ditugaskan sebagai Verifikator Soal.`
                    : 'Dosen berhasil ditugaskan sebagai Verifikator Soal.'
            );
    ...
    const handleOpenDelete = (item: Penugasan) => {
        setDeleteId(item.id);
        setDeleteTargetName(
    ...
    // Coordinated courses list from API meta (for Koordinator MK) or all courses (for Super Admin)
    const filterCourseOptions = isSuperAdmin
        ? allCourses
        : (response?.meta?.coordinated_courses || []);

    const isKoordinatorView = !isSuperAdmin;

    if (!isSuperAdmin && !isKoordinatorMk) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] text-center p-6">
                ...
                <p className="text-sm text-gray-500 max-w-md mt-1 mb-6 leading-relaxed">
                    Fitur Monitoring Verifikator hanya tersedia untuk Dosen dengan role <strong>Koordinator MK</strong> atau <strong>Super Administrator</strong>.
                </p>
                <Link
                    to="/dashboard"
                    className="inline-flex items-center gap-2 rounded-xl bg-[var(--color-primary)] px-5 py-2.5 text-sm font-semibold text-white shadow transition hover:bg-[var(--color-primary-dark)]"
                >
    ...
    return (
        <div className="flex flex-col gap-6">
            <PageHeader
                title={isKoordinatorView ? 'Monitoring Verifikator Soal' : 'Penugasan Verifikator Soal'}
                description={
                    isKoordinatorView
                        ? 'Pantau seluruh Dosen Verifikator yang ditunjuk oleh Super Admin untuk mata kuliah yang Anda koordinasikan pada periode aktif.'
                        : 'Tugaskan Dosen sebagai Verifikator Soal yang bertanggung jawab memverifikasi berkas naskah soal ujian dalam periode aktif.'
                }
                breadcrumb={[
                    { label: isKoordinatorView ? 'Monitoring Verifikator Soal' : 'Penugasan Verifikator Soal' }
                ]}
                action={
                    isSuperAdmin ? (
                        <button
                        ...
                        >
                            <Plus size={16} />
                            Tugaskan Verifikator Soal
                        </button>
                    ) : undefined
                }
            />

            {/* ── Mode Pemantauan Koordinator MK Banner ── */}
            {isKoordinatorView && (
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 rounded-2xl border border-blue-200 bg-gradient-to-r from-blue-50/90 via-indigo-50/50 to-blue-50/90 p-4 shadow-xs">
                    <div className="flex items-center gap-3">
                        <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-blue-600 text-white shadow-sm shrink-0">
                            <ShieldCheck size={20} />
                        </div>
                        <div>
                            <p className="text-sm font-bold text-blue-950">Mode Pemantauan Koordinator Mata Kuliah</p>
                            <p className="text-xs text-blue-700 mt-0.5">
                                Menampilkan seluruh verifikator yang ditunjuk oleh Super Admin untuk mata kuliah yang Anda koordinasikan.
                            </p>
                        </div>
                    </div>
                    ...
                    {filterCourseOptions.length > 0 && (
                        <div className="flex items-center gap-1.5 flex-wrap">
                            <span className="text-xs font-semibold text-blue-900">Mata Kuliah Dipegang:</span>
                            {filterCourseOptions.map((c: any) => (
                                <span
                                    key={c.id}
                                    className="rounded-lg bg-white border border-blue-200 px-2.5 py-1 font-mono text-xs font-bold text-blue-800 shadow-2xs"
                                >
                                    {c.kode_mk} · {c.nama_mk}
                                </span>
                            ))}
            ...
            <FilterBar onReset={handleReset}>
                ...
                {filterCourseOptions.length > 0 && (
                    <div className="flex items-center gap-2 ml-2">
                        ...
                        >
                            <option value="">
                                {isKoordinatorView ? 'Semua Mata Kuliah Koordinasi' : 'Semua Mata Kuliah'}
                            </option>
                            {filterCourseOptions.map((c: any) => (
                    ...
                    <table className="w-full border-collapse text-left text-sm text-gray-600">
                        <thead className="border-b border-gray-200 bg-gray-50 text-xs font-semibold uppercase tracking-wider text-gray-700">
                            <tr>
                                ...
                                <th className="px-6 py-4">Dosen Verifikator</th>
                                ...
                                <th className="px-6 py-4 w-32 text-center">
                                    {isSuperAdmin ? 'Aksi' : 'Status'}
                                </th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-200">
                            {isLoading && <SkeletonTable rows={5} cols={isSuperAdmin ? 8 : 7} />}
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                <div className="flex h-9 w-9 items-center justify-center rounded-full bg-[var(--color-primary-light)] text-xs font-bold text-[var(--color-primary)]">
                                                    {r.dosen?.nama_lengkap?.charAt(0).toUpperCase() || 'V'}
                                                </div>
                                            ...
                                            {isSuperAdmin ? (
                                                <div className="flex items-center justify-center">
                                                    <button
                                                        ...
                                                        title="Cabut Penugasan Verifikator"
                                                    >
                                                        <MinusCircle size={16} />
                                                    </button>
                                                </div>
                    ...
                    <EmptyState
                        ...
                        description={
                            debouncedSearch
                                ...
                                : isKoordinatorView
                                ? 'Belum ada Dosen Verifikator yang ditugaskan oleh Super Admin untuk mata kuliah yang Anda koordinasikan pada periode ini.'
                                : 'Belum ada dosen yang ditugaskan sebagai Verifikator Soal pada periode ini.'
                        }
                    />
                )}
            ...
            {isSuperAdmin && (
                <>
                    ...
                    <ConfirmDialog
                        ...
                        title="Cabut Penugasan Verifikator"
                        message={`Apakah Anda yakin ingin mencabut penugasan verifikator untuk ${deleteTargetName}? Dosen tidak akan lagi dapat memverifikasi soal pada mata kuliah tersebut. Tindakan ini dapat dibatalkan dengan menugaskannya kembali.`}
